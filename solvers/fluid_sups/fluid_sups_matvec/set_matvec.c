...
void HROM_ddecm_set_reduced_vec_para3(
    MONOLIS*        monolis,
    BBFE_DATA*      fe,
    VALUES*         vals,
    BBFE_BASIS*     basis,
    ...
    double          t)
{
    (void)monolis; (void)hr_vals; (void)hlpod_vals; (void)num_modes; (void)dt; (void)t;
    ...
    double* wJ  = (double*)malloc(sizeof(double)*(size_t)np);
    double* tau = (double*)malloc(sizeof(double)*(size_t)np);

    // ============================================================
    // ★ 修正点：pass(内部/D境界) を廃止し、全要素 fe->total_num_elems を1回だけ回す
    // ★ 重みは常に 1.0
    // ============================================================
    for (int e = 0; e < fe->total_num_elems; ++e) {
        ...
        for (int i = 0; i < nl; ++i) {
            ...
            const int sid = hlpod_mat->subdomain_id_in_nodes[gi];
            if (sid >= num_subdomains) continue;

            const int IS  = hlpod_ddhr->num_neib_modes_1stdd_sum[sid];
            const int IE  = hlpod_ddhr->num_neib_modes_1stdd_sum[sid + 1];
            const int msz = IE - IS;
            if (msz <= 0) continue;

            // accv(4) = ∑p vec(4,p) * wJ[p]
            double accv[4] = {0,0,0,0};
            for (int p = 0; p < np; ++p) {
                double vec[4];
                BBFE_elemmat_fluid_sups_vec(
                    vec, basis->N[p][i], fe->geo[e][p].grad_N[i],
                    v_ip[p], vals->density, tau[p], vals->dt);
                const double wp = wJ[p];
                accv[0]+=vec[0]*wp; accv[1]+=vec[1]*wp; accv[2]+=vec[2]*wp; accv[3]+=vec[3]*wp;
            }

            // reduced_RH[IS:IE) に列ごと DAXPY で直接加算（重み=1）
            const double elem_w = 1.0;
            for (int a = 0; a < 4; ++a) {
                const double alpha = elem_w * accv[a];
                const double* __restrict col_a = &hlpod_mat->pod_modes[gi*4 + a][IS];
                cblas_daxpy(msz, alpha, col_a, 1,
                            //&hlpod_ddhr->reduced_RH[IS], 1);
                            &hlpod_mat->VTf[IS], 1);
            }
        } // i
    } // e
