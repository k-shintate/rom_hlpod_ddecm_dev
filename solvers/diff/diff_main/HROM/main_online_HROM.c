		...
		char* argv[])
{
	...
	while (t < sys.vals.rom_finish_time) {
        ...
        if(step%sys.vals.output_interval == 0) {
            ...
            ROM_std_hlpod_output_add_calc_time(sys.hrom.hr_vals.time_calc_reduced_matvec, t,
					"calctime/time_calc_reduced_mat.txt", sys.cond.directory);
            ROM_std_hlpod_output_add_calc_time(sys.hrom.hr_vals.time_linear_solver, t,
