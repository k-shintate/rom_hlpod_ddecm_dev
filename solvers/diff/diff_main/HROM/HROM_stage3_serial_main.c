...
#include "hrom_nnls_svd_preprocess.hpp"
...
{
    if (argc<3) {
        fprintf(stderr,"usage: %s <directory> <num_rank_files> [max_iter=1000] [tol=1e-8] [weight_tol=1e-8] [svd_rtol=1e-10]\n",argv[0]);
        return EXIT_FAILURE;
    }
    ...
    const double svd_rtol=(argc>6)?atof(argv[6]):1.0e-10;
    ...
    printf("============================================================\n");
    printf("[Stage 3 v9: global-key + summed-rank assembly + SVD]\n");
    printf("rank files                      = %d\n",num_ranks);
    ...
    printf("SVD relative cutoff             = %.3e\n",svd_rtol);
    printf("============================================================\n");

    HROM_NNLS_SVD_REDUCTION svd =
        hrom_nnls_svd_reduce_rows(A,b.data(),m,n,svd_rtol);

    if (svd.reduced_rows <= 0 || svd.A == NULL || svd.b == NULL) {
        die("Stage-3 SVD preprocessing failed");
    }

    hrom_print_svd_summary("Stage 3",svd);

    double svd_solver_residual=0.0;
    monolis_optimize_nnls_R_with_sparse_solution_initial_set(
        svd.A,svd.b,x.data(),svd.reduced_rows,n,
        max_iter,tol,active.data(),&svd_solver_residual);

    /* Always evaluate accuracy in the original normalized Stage-3 system. */
    const double residual =
        hrom_nnls_residual_l2(A,b.data(),x.data(),m,n);
    ...
    printf("[Stage 3 SVD residual check] reduced_solver=%.15e original_normalized=%.15e raw=%.15e\n",
           svd_solver_residual,residual,raw_residual);
     ...
     */
    {
        ...
        fprintf(
            fp_report,
            "svd_sigma_min_retained %.30e\n",
            svd.sigma_min_retained);
        fprintf(
            fp_report,
            "svd_condition_estimate %.30e\n",
            svd.condition_estimate);
        fprintf(
            fp_report,
            "svd_discarded_energy_percent %.15e\n",
            100.0 * svd.discarded_sigma_energy_ratio);
        fprintf(
            fp_report,
            "svd_solver_residual %.30e\n",
            svd_solver_residual);
        fprintf(
            fp_report,
    ...
    printf("[Stage 3 v9 SVD result] selected=%d / %d "
