...
static std::vector<RouteEntry> read_all_routes(const char* directory, int num_ranks)
{
    ...
    for (int rank=0; rank<num_ranks; ++rank) {
        ...
        for (int i=0; i<n; ++i) {
            ...
            int s=-1;
            if (fscanf(fp, "%lld %lld %d", &original, &parallel, &s)!=3) {
                fclose(fp); die("invalid route entry");
            }
            RouteEntry e;
            e.rank=rank;
    ...
    const std::vector<std::pair<int64_t,double>>& selected)
{
    ...
    for (int s: subdomains) {
        read_class_file(directory, s, false, class_by_sub_parallel, global_class_by_parallel);
        read_class_file(directory, s, true, class_by_sub_parallel, global_class_by_parallel);
    }
    ...
    /* Resolve every class before touching any legacy file. */
    struct Resolved {
        RouteEntry route;
        int dbc;
        double w;
    ...
    for (const auto& p: selected) {
        ...
        if (it==routes_by_original.end()) {
            fprintf(stderr, "ERROR: final original-global element %lld has no route\n", (long long)original);
            exit(EXIT_FAILURE);
        }

        int fallback_class=-1;
        /* First find a class from any exact Stage-2 occurrence of this physical element. */
        for (const RouteEntry& e: it->second) {
            const unsigned long long packed=
                ((unsigned long long)(unsigned int)e.fine_subdomain<<32) |
                (unsigned long long)(unsigned int)e.parallel_elem_id;
            auto ci=class_by_sub_parallel.find((long long)packed);
            if (ci!=class_by_sub_parallel.end()) {
                if (ci->second==1) fallback_class=1;
                else if (fallback_class<0) fallback_class=0;
            }
        }
        if (fallback_class<0) {
            fprintf(stderr,
                "ERROR: cannot recover D_bc class for final original-global element %lld from Stage-2 files\n",
                (long long)original);
            exit(EXIT_FAILURE);
        }

        for (const RouteEntry& e: it->second) {
            int cls=fallback_class;
            const unsigned long long packed=
                ((unsigned long long)(unsigned int)e.fine_subdomain<<32) |
                (unsigned long long)(unsigned int)e.parallel_elem_id;
            auto ci=class_by_sub_parallel.find((long long)packed);
            if (ci!=class_by_sub_parallel.end()) cls=ci->second;
            Resolved rr; rr.route=e; rr.dbc=cls; rr.w=w;
            resolved.push_back(rr);
        }
    }

    for (const Resolved& rr: resolved) {
        const int s=rr.route.fine_subdomain;
        const long long id=(long long)rr.route.parallel_elem_id;
        if (rr.dbc) {
            if (seen_dbc[s].insert(id).second) dbc_out[s].push_back({id,rr.w});
        } else {
            if (seen_no[s].insert(id).second) no_out[s].push_back({id,rr.w});
        }
    }
    ...
    for (int s: subdomains) {
        ...
        copy_file_if_missing(no_path, no_path+".pre_stage3_v7.bak");
        copy_file_if_missing(db_path, db_path+".pre_stage3_v7.bak");

        std::sort(no_out[s].begin(), no_out[s].end(), [](const OutEntry&a,const OutEntry&b){return a.id<b.id;});
        std::sort(dbc_out[s].begin(), dbc_out[s].end(), [](const OutEntry&a,const OutEntry&b){return a.id<b.id;});
        ...
        printf("[legacy-write] subdomain=%d D_bc=%zu no_D_bc=%zu total=%zu\n",
            s, dbc_out[s].size(), no_out[s].size(), dbc_out[s].size()+no_out[s].size());
    }

    printf("[legacy-write] unique physical Stage-3 elements=%zu distributed exact routes=%zu\n",
        selected.size(), resolved.size());
}
...
{
    if (argc<3) {
        fprintf(stderr,"usage: %s <directory> <num_rank_files> [max_iter=1000] [tol=1e-8] [weight_tol=1e-8]\n",argv[0]);
        return EXIT_FAILURE;
    }
    const char* directory=argv[1];
    const int num_ranks=atoi(argv[2]);
    const int requested_iter=(argc>3)?atoi(argv[3]):1000;
    const int max_iter=(requested_iter>0)?std::min(requested_iter,1000):1000;
    const double tol=(argc>4)?atof(argv[4]):1.0e-8;
    const double weight_tol=(argc>5)?atof(argv[5]):tol;
    if (num_ranks<=0) die("num_rank_files must be positive");
    ...
    for (const auto& kv:candidate_seed_weight) candidates.push_back(kv.first);
    std::sort(candidates.begin(),candidates.end());

    const int m=(int)rows.size();
    const int n=(int)candidates.size();
    if (m<=0 || n<=0) die("empty Stage-3 system");
    ...
    int nactive=0; for(int v:active) if(v)++nactive;
    printf("============================================================\n");
    printf("[Stage 3 v7: global-key + summed-rank assembly]\n");
    printf("rank files                      = %d\n",num_ranks);
    ...
    monolis_optimize_nnls_R_with_sparse_solution_initial_set(
        A,b.data(),x.data(),m,n,max_iter,tol,active.data(),&residual);

    std::vector<std::pair<int64_t,double>> selected;
    for (int c=0;c<n;++c) if (x[(size_t)c]>weight_tol) selected.emplace_back(candidates[(size_t)c],x[(size_t)c]);
    ...
    const double stage2_union_retained_percent =
        (cumulative_num_original > 0)
        ? 100.0 * (double)stage3_num_candidates
            / (double)cumulative_num_original
        : 0.0;

    const double stage2_union_reduction_percent =
        (cumulative_num_original > 0)
        ? 100.0 * (double)(cumulative_num_original-stage3_num_candidates)
            / (double)cumulative_num_original
        : 0.0;
    ...
    printf("Stage-3 retained vs. Stage-2      = %.3f %%\n",
        stage3_retained_percent);
    printf("Stage-3 reduction vs. Stage-2     = %.3f %%\n",
        stage3_reduction_percent);
     ...
     */
    {
        ...
        fprintf(
            fp_report,
            "stage3_reduction_vs_stage2_percent %.15e\n",
            stage3_reduction_percent);
        fprintf(
            fp_report,
    ...
    printf("[Stage 3 v7 result] selected=%d / %d "
