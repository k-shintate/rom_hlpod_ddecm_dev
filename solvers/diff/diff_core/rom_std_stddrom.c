    ...
    int communicator)
{
    ...
    snapmat = stdd_alloc_matrix_rows(
        system->internal_st_dof,
        num_snapshot_columns);
    left_vectors = stdd_alloc_matrix_rows(
        system->internal_st_dof,
        num_snapshot_columns);
    right_vectors = stdd_alloc_matrix_rows(
        num_snapshot_columns,
        num_snapshot_columns);
    ...
    if(snapmat == NULL || left_vectors == NULL || right_vectors == NULL ||
    ...
    monolis_scalapack_comm_initialize(self_comm, &scalapack_comm);
...
cleanup:
    ...
    stdd_free_matrix_rows(&snapmat);
    stdd_free_matrix_rows(&left_vectors);
    stdd_free_matrix_rows(&right_vectors);
