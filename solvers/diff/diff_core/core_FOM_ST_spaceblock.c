...
/*
 * Algebraic-unknown contract used by the DDROM driver.
 *
 * This STSB implementation solves directly for the physical current state
 * U^n.  A native incremental FOM implementation should return 1 here and
 * make STSB_solve_window() return Delta U^n.
 */
#ifdef __cplusplus
extern "C"
#endif
    ...
    int* num_internal_space_nodes)
{
    ...
    *num_windows = STSB_get_num_windows(
    ...
    /*
     * The communication table is the authoritative source of ownership.
     *
     * BBFE_convdiff_pre() may leave sys->monolis.mat.N equal to the number
     * of local vertices (owned + halo).  Using that value as the owned-row
     * count duplicates halo rows in snapshots/POD inner products and makes
     * the ST-DDROM basis incompatible with the verified HLPOD kernels.
     * MONOLIS stores owned vertices first and halo vertices afterwards; the
     * number of owned vertices is monolis_com.n_internal_vertex.
     */
    {
