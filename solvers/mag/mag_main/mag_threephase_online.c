...
void ROM_sys_hlpod_fe_write_local_pod_modes_B(
        FE_SYSTEM		sys,
        const char*		fn1,
        double**		pod_modes,
...
{
    double* vec;
    vec = BB_std_calloc_1d_double(vec, sys.fe.total_num_nodes);

    for(int i = 0; i < n_internal_vertex; i++){
        vec[i] = pod_modes[i][1];
    }

    for(int e=0; e<sys.fe.total_num_elems; ++e){
        for(int i=0; i<sys.ned.local_num_edges; ++i){
            int gi = sys.ned.nedelec_conn[e][i];
            
            vec[gi] = pod_modes[gi][0];
        }
    }

    char fnode[BUFFER_SIZE];
    snprintf(fnode, BUFFER_SIZE, "pod_modes_vtk/pod_modes_B.vtk");
    const char* fn2 = monolis_get_global_output_file_name(MONOLIS_DEFAULT_TOP_DIR, "./", fnode);
    output_B_node_vtk(&(sys.fe), &(sys.basis), &(sys.ned), vec, fn2, sys.cond.directory);

    BB_std_free_1d_double(vec, sys.fe.total_num_nodes);
}
		...
		char* argv[])
{
    ...
    */
    ...
    ROM_sys_hlpod_fe_write_pod_modes_vtk_Aphi(
		...
		1,
		1,
		"pod_modes_vtk/pod_modes_A.vtk",
		"pod_modes_vtk/pod_modes_phi.vtk",
		sys.cond.directory);
    */
    /*
    ROM_sys_hlpod_fe_write_local_pod_modes_B(
        sys,
        "pod_modes_vtk/pod_modes_B.vtk",
...
*/
	...
	if(monolis_mpi_get_global_my_rank() == 0){
		fp = BBFE_sys_write_fopen(fp, "l2_error_B.txt", sys.cond.directory);
		fclose(fp);
    ...
    printf("HROM_std_hlpod_online_pre\n");
