...
void solver_fom_NR_Aphi_team21a(
    FE_SYSTEM sys,
    double t,
    int step,
    double* x_prev,
...
void solver_fom_NR_Aphi_team21c_collect_snapmat2(
    FE_SYSTEM sys,
    double t,
    int step,
    double* x_prev,
    ...
    int total_num_dof)
{
   ...
   //double* rvec = (double*)calloc((size_t)sys.ned.total_num_dof, sizeof(double));
    ...
    double* dx = BB_std_calloc_1d_double(dx,   sys.ned.total_num_dof);
    double* rvec = BB_std_calloc_1d_double(rvec, sys.ned.total_num_dof);
    double* rvec_old = BB_std_calloc_1d_double(rvec_old, sys.ned.total_num_dof);
	...
	if(monolis_mpi_get_global_my_rank()==1){
		//add_phi_gauge_bc_team21a0(&(sys.monolis), &(sys.fe), &(sys.ned), &(sys.bc));
	}
        /* 残差ベクトルを保存（B = -F） */
        if(it==0){
...
*/
    ...
    BB_std_free_1d_double(dx,   sys.ned.total_num_dof);
    BB_std_free_1d_double(rvec,   sys.ned.total_num_dof);
    BB_std_free_1d_double(rvec_old,   sys.ned.total_num_dof);
