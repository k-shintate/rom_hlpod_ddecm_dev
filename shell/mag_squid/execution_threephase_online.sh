...
cp -r mag_threephase_online ./../../$directory

cd ./../../$directory

mkdir -p {pod_modes_vtk,pod_modes,fem_solver_prm,pod_solver_prm,calctime,hot_start}
for ((i=0; i<nd; i++))
do
...
echo "exit" >> $fname
mpirun -np ${np}  gdb --command=gdb_cmd ./mag_threephase_online
