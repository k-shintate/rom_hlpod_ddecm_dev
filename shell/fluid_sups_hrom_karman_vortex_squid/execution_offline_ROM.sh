...
fname="execution_hrom_offline_ROM_karman_vortex.sh"
rm $fname
touch $fname
echo "#!/bin/bash" >> $fname
...
echo "#PBS -l elapstim_req=01:00:00" >>$fname            # debug == max 0:10:00, regular == max 24:00:00
...
echo "mpirun \${NQSV_MPIOPTS} -np ${np} ./hlpod_fluid_sups_offline_ROM ./ -nd ${nd} -nm ${nm} -pa 0.9999999 -st ${st} -hs > ../../result/result_offline_ROM_${nd}-${nm}.txt" >> $fname
#echo "mpirun \${NQSV_MPIOPTS} -np ${np} ./hlpod_fluid_sups_offline_ROM ./ -nd ${nd} -nm ${nm} -pa 0 -st ${st} -hs > ../../result/result_offline_ROM_${nd}-${nm}.txt" >> $fname
