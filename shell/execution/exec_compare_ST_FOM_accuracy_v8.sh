...
# Latest ST-FOM v8 comparison runner
# Fixed spatial resolution; compare dG(0), dG(1), dG(2)
# at dt = 0.01 and 0.05 using final-time accuracy.
#
# Usage:
#   bash shell/execution/exec_compare_ST_FOM_accuracy_v8.sh <e> <ep> <np> [finish_time] [slabs_per_window]
#
# Example:
#   bash shell/execution/exec_compare_ST_FOM_accuracy_v8.sh 20 1.0 4 1.0 4
...
PLOTTER="$ROOT_DIR/shell/plot_st_accuracy_compare_v8.py"
SOLVER_SRC="$ROOT_DIR/solvers/diff/diff_core/core_FOM_ST_spaceblock.c"

if [[ ! -f "$RUNNER" ]]; then
    echo "ERROR: base ST-FOM runner not found: $RUNNER" >&2
    exit 1
fi

if [[ ! -f "$SOLVER_SRC" ]]; then
    echo "ERROR: ST-FOM solver source not found: $SOLVER_SRC" >&2
    exit 1
fi

# Verify that the history-fix generation is installed.
if ! grep -q "ST HISTORY step=" "$SOLVER_SRC"; then
    echo "ERROR: v8 history-fix solver does not appear to be installed." >&2
    echo "       Expected marker 'ST HISTORY step=' in:" >&2
    echo "       $SOLVER_SRC" >&2
    exit 1
fi
...
echo "============================================================"
echo " Latest ST-FOM v8 final-time accuracy comparison"
echo "------------------------------------------------------------"
...
for dg in "${DEGREES[@]}"; do
    for dt in "${DTS[@]}"; do
        ...
        if (( steps % SW != 0 )); then
            echo "ERROR: total steps $steps is not divisible by slabs/window $SW (dt=$dt)" >&2
            exit 1
        fi
        ...
        # Keep the numerical setup identical except for temporal degree and dt.
...
echo " observed order    : $ORDER_CSV"
echo " relative L2 plot  : $COMPARE_DIR/final_relative_l2_by_degree.png"
echo " relative L2 log   : $COMPARE_DIR/final_relative_l2_by_degree_log.png"
