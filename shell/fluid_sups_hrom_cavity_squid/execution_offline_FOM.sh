...
fname="execution_hrom_offline_FOM_cavity.sh"
rm $fname
touch $fname
echo "#!/bin/bash" >> $fname
...
echo "mpirun \${NQSV_MPIOPTS} -np ${np} ./hlpod_fluid_sups_offline_FOM ./ -nd ${nd} -nm ${nm} -pa 0.9999 -st ${st}  > ../../result/result_offline_FOM_${nd}-${nm}.txt" >> $fname
