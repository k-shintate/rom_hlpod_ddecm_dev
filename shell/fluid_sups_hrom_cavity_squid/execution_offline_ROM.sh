...
make -f Makefile_HROM_cavity_squid clean
make -f Makefile_HROM_cavity_squid
cd ../..

fname="execution_hrom_offline_ROM_cavity.sh"
rm $fname
touch $fname
echo "#!/bin/bash" >> $fname
...
echo "#PBS -l elapstim_req=12:00:00" >>$fname            # debug == max 0:10:00, regular == max 24:00:00
echo "#PBS -l cpunum_job=${N_cpu}" >> $fname            # CPU/node == max 72
echo "#PBS -l memsz_job=248GB" >> $fname
echo "#PBS -M k.shintate.hpc@gmail.com" >> $fname
echo "#PBS -m eb" >> $fname
...
echo "mpirun \${NQSV_MPIOPTS} -np ${np} ./hlpod_fluid_sups_offline_ROM ./ -nd ${nd} -nm ${nm} -pa 0 -st ${st}  > ../../result/result_offline_ROM_${nd}-${nm}.txt" >> $fname
