...
!
! MPI 並列時:
!  - metagraph.dat.n_internal から、この rank が読む domain 数 n_graphs を取得する
!  - metagraph.dat.id.<rank> から domain_id list を読み、入力 domain を選んでマージする
!
...
program merge_test_cio_full
  ...
  !  - MPI 並列時は metagraph.dat.n_internal から、この rank が読む domain 数を取得する
  !============================================================
  ...
  if(nprocs == 1) then
    ...
    if(.not. ex) then
      write(*,*) "Error: metagraph.dat.n_internal not found: ", trim(fname)
      call monolis_mpi_finalize()
      stop
    endif
  ...
  ! 貼り付けテキスト（2点）側と同じく、MPI 並列時は
  ! metagraph.dat.id.<rank> の先頭2行をスキップして domain_id list を読む。
