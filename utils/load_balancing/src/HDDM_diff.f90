...
!
! 変更点:
!  - graph_elem_bool.dat / nedelec_edge_sign.dat は「マージしない」ため、一切読まない・出力しない
...
program merge_test_cio_full
  ...
  integer(kint) :: nnode_node, nedge_local
  ...
  do i = 1, n_graphs
    ...
    !> ===== connectivity (graph_elem) =====
    fname = './parted.0/parted.1/graph_elem.dat.' // trim(cnum)
    ...
    fname = './parted.0/parted.1/graph_elem.dat.n_internal.' // trim(cnum)
    ...
    fname = './parted.0/parted.1/graph_elem.dat.id.' // trim(cnum)
    ...
    !============================================================
    ! graph_elem_bool.dat / nedelec_edge_sign.dat はマージしないため処理しない
    !============================================================
...
contains
  ...
  subroutine output_all_c(outdir, gnod, gcon, com, nnode, ndof, node, nbc, ibc, rbc, nload, iload, rload, is_c0, n_internal_out)
    ...
    fname = monolis_get_global_output_file_name(outdir, "", "graph_elem.dat")
    call monolis_output_graph(fname, gcon%n_vertex, gcon%vertex_id, index_tmp, item_tmp)

    fname = monolis_get_global_output_file_name(outdir, "", "graph_elem.dat.n_internal")
    call monolis_output_internal_vertex_number(fname, gcon%n_internal_vertex)

    fname = monolis_get_global_output_file_name(outdir, "", "graph_elem.dat.id")
